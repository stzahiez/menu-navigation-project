...
#   set_location_assignment  PIN_P26 -to EXT_CLOCK
   ...
   set_location_assignment PIN_G26  -to right  ;# KEY0
   set_location_assignment PIN_N23  -to left  ;# KEY1
   set_location_assignment PIN_P23  -to up  ;# KEY2
   set_location_assignment PIN_W26  -to down  ;# KEY3
   ...
   set_location_assignment PIN_N25  -to fpga_rst   ;# SW0
...
#   set_location_assignment PIN_AE15  -to LEDR13  ;# LEDR13
  ...
  set_location_assignment PIN_N9   -to msb_version[0]   ;# g HEX7S[0]
   ...
   ;# httptechtrain.microchip.comwebseminarsdocumentsIrDA_BW.pdf
...
#   set_location_assignment PIN_AE25 -to IRDA_RXD
   ...
   ;# using Wolfson WM8731 audio CODEC (enCOderDECoder)
   ;# This chip supports microphone-in, line-in, and Line-outPhones-out
   ;# ports (we only have Phones-out),
   ;# Sample rate adjustable from 8 kHz to 96 kHz.
   ;# The WM8731 is controlled by a serial I2CSPI bus interface (We only have I2C).
   ...
   ;# 1 - start command
   ;# 2 - device addwrite prefix 00110100 (adrres 0011010 & write command)
   ;#                               7654321W
   ;# 3 - Check Acknowledge from device addwrite prefix
   ;# 4 - First byte              RRRRRRRD (register addressRRRRRRR & MSB of data D)
   ;#                               65432107
...
#   set_location_assignment PIN_B7   -to VGA_SYNC
   ...
   ;# into 422 component video data compatible with 16-bit8-bit CCIR601CCIR656.
   ;# The ADV7181 is compatible with a broad range of video devices, including DVD
   ;# players, tape-based sources, broadcast sources, and securitysurveillance cameras.
...
#   set_location_assignment PIN_K2 -to LCD_BLON
   ...
   ;# a media access control (MAC) unit, and a 10100M PHY transceiver.
...
#   set_location_assignment PIN_B24 -to ENET_CLK
   ;# DM9000A CommandData Select,0 =Command,1 =Data
...
#   set_location_assignment PIN_W23  -to JP2_PIN40
   ...
   ;# at full-speed (12 Mbits) and low-speed (1.5 Mbits).
